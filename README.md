...
Phishing Attack Defender combines rule-based URL inspection, machine learning model predictions, email link extraction, domain checking, and takedown request generation in a beautiful, production-ready package.
...
- Analyze one domain for phishing risk
...
### 2. **Email Scanner** (Dual ML)
- **NEW**: Direct **email content ML classification** (urgency words, spoofing, panic punctuation)
- Extracts & scans all URLs (existing ML + rules)
- **Dual verdict**: Email ML + URL ML → overall risk
- Summary shows ML confidences + counts 🎯

### 3. Domain Checker
- Accepts one domain as input
- Uses domain-specific phishing analysis, plus ML confidence blending
- Flags brand impersonation, risky TLDs, tunnel domains, punycode, and suspicious structure
...
### `POST /check-domain`
Checks whether a domain appears to be phishing.
...
Legacy compatibility endpoint: `POST /check-typosquat` (same request format as `check-domain`).
